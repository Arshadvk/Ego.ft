...
const editing_coupon = async (req, res) => {
  try {
    ...
    const upadate = await Coupon.updateOne({ _id: id }, {
      $set: {
        coupon_name: name,
        code: code,
        expiry_date: date,
        ...
        if (currentdate <= coupondata.expiry_date) {
          ...
          if (coupondata.min_amount <= cartTotel) {
            ...
            if (discountAmount <= coupondata.maxdiscountprice) {
              ...
              const coupon_apply = await User.updateOne({_id : req.session.user._id},{$set:{cartTotel:value}})

              const coupon_used = await Coupon.updateOne({code : code},{$push:{used:id}})
...
const place_order = async (req, res) => {
  ...
  try {
    ...
    const proid = await User.aggregate([
      // Match documents that match a certain condition, if necessary
      { $match: { _id: id } },

      // Project only the "product id" field from the "cart" array
    ...
    for (let i = 0; i < orderData.productId.length; i++) {
      ...
      productPush.push({ productid: productssid, qty: quantitys, singleTotel: singleTotels, singlePrice: price })
    }

    // status updating 
    const status = req.body.shipping_method == "COD" ? "Confirmed" : "Pending"
    ...
    const order = new Order({
      ...
      product: productPush,
      totel: totel,
      paymentType: req.body.shipping_method,
      status: status,
      orderId: orderId
    ...
    if (req.body.shipping_method == 'COD') {

      res.json({ status: true })


    } else if (req.body.shipping_method == 'UPI') {
...
const verify_payment = async (req, res) => {
  try {
    ...
    const upadateOrder = await Order.updateOne({ orderId: latestOrder.orderId }, { $set: { status: "Confirmed" } })
...
const order_success = async (req, res) => {
  try {
    ...
    const removeing = await User.updateOne({ _id: user }, { $set: { cart: [], cartTotel: 0 } })

    const order = await Order.findOne().sort({ date: -1 }).populate('product.productid')

    for (let i = 0; i < order.product.length; i++) {
      await Product.updateOne({ _id: order.product[i].productid }, { $inc: { quantity: -order.product[i].qty } })
    }
...
const cancel_order = async (req, res) => {
  try {
    ...
    if (orderdata.paymentType == 'UPI') {
      const refund = await User.updateOne({ _id: userid }, { $inc: { wallet: orderdata.totel } })
    }
...
const retrun_order = async (req, res) => {
  try {
    ...
    const retrun_order = await Order.updateOne({ _id: order_id }, { $set: { status: "Return Pending" } })
    res.json({ success: true })
...
const confirm_return = async (req, res) => {
  try {
    ...
    const upate = await Order.updateOne({ _id: orderId }, { $set: { status: status } })
...
const show_orderlist = async (req, res) => {
  ...
  try {
    ...
    const category = await Category.find()
    res.render('list-orders', { user: true, category, orders });
...
const view_order_admin = async (req, res) => {
  ...
  try {
    ...
    const order = await Order.find({ _id: order_id }).populate('product.productid')
